`default_nettype none
`timescale 1ns/1ns

// =============================================================================
// WEIGHT FIFO — Double-Buffered Weight Staging
// =============================================================================
//
// PROBLEM: 
//   In a TPU, loading weights from memory is slow, but computation is fast. 
//   If we use one buffer, we have to stop the array while we load the next tile.
//
// SOLUTION: 
//   Double-Buffering (Ping-Pong). While the Systolic Array drains data from 
//   "Active Buffer", a background process (Prefetcher) fills the "Inactive Buffer"
//   with the next tile. When both are ready, we swap them instantly.
//
// ARCHITECTURE:
//   Two independent processes running in parallel:
//   1. PREFETCH FSM : DMA-like engine that reads from SRAM -> fills Inactive Buffer.
//   2. DRAIN LOGIC  : Streams rows from Active Buffer -> Systolic Array.
// =============================================================================

module weight_fifo #(
    parameter N            = 8,   // Systolic array dimension (NxN PEs)
    parameter DATA_WIDTH   = 8,   // INT8 weights
    parameter BUFFER_DEPTH = 64   // N×N = 64 weights per buffer
)(
    input wire clk,
    input wire reset,

    // -----------------------------------------------------------------------
    // Memory Interface — connects to unified_buffer via memory_controller
    // -----------------------------------------------------------------------
    output reg          mem_req,    // Pulse high to request one 32-bit word
    output reg  [13:0]  mem_addr,   // Word address in unified buffer
    input wire  [31:0]  mem_rdata,  // 32-bit response (holds 4 INT8 weights)
    input wire          mem_valid,  // Memory controller: "data is ready this cycle"
    output wire         mem_busy,   // Signals that prefetch is active

    // -----------------------------------------------------------------------
    // Prefetch Control — tells the module what tile to load
    // -----------------------------------------------------------------------
    input wire          prefetch_start,      // Pulse: begin fetching a new tile
    input wire  [13:0]  prefetch_base_addr,  // Start address of tile in unified buffer
    input wire  [7:0]   prefetch_rows,       // How many rows to fetch (up to N)
    output wire         prefetch_done,       // High for one cycle when fetch complete
    output wire         prefetch_busy,       // High while fetch is in progress

    // -----------------------------------------------------------------------
    // Systolic Array Interface — feeds weights row by row into the PE grid
    // -----------------------------------------------------------------------
    input wire          drain_enable,    // Array is ready to receive weights
    input wire          drain_row_done,  // Array: "finished loading current row, next please"
    output wire signed [DATA_WIDTH-1:0] weight_out [N-1:0], // N weights for current row
    output wire [N-1:0] weight_row_select, // One-hot: which row of PEs to load
    output wire         weight_valid,      // weight_out is valid this cycle
    output wire         buffer_empty,      // Active buffer has been fully drained
    output wire         buffer_ready,      // Active buffer has data to send

    // -----------------------------------------------------------------------
    // Status / Debug
    // -----------------------------------------------------------------------
    output wire [1:0]   active_buffer,  // 0=A active, 1=B active
    output wire [5:0]   fill_level_a,   // Count of words loaded in Buffer A
    output wire [5:0]   fill_level_b
);

    // =========================================================================
    // BLOCK A — STORAGE & STATE
    // =========================================================================

    // Double buffer storage: Two independent NxN grids
    // buffer[row][col]
    reg signed [DATA_WIDTH-1:0] buffer_a [0:N-1][0:N-1];
    reg signed [DATA_WIDTH-1:0] buffer_b [0:N-1][0:N-1];

    // Control registers
    reg        active_buf;      // 0 = array reads A, 1 = array reads B
    reg [2:0]  drain_row;       // Row index being sent to array (0 to N-1)

    reg        buffer_a_valid;  // Buffer A has a complete tile
    reg        buffer_b_valid;  // Buffer B has a complete tile
    reg [5:0]  words_in_a;      // Diagnostics: word count in A
    reg [5:0]  words_in_b;      // Diagnostics: word count in B

    // Prefetch FSM states
    localparam PF_IDLE    = 3'b000;
    localparam PF_REQUEST = 3'b001;
    localparam PF_WAIT    = 3'b010;
    localparam PF_STORE   = 3'b011;
    localparam PF_DONE    = 3'b100;

    reg [2:0]  pf_state;
    reg [13:0] pf_addr;
    reg [5:0]  pf_word_count;   // Words fetched in current prefetch
    reg [5:0]  pf_total_words;  // Total words expected for the tile
    reg        pf_target_buffer;// Which buffer we are currently filling

    // Position tracking for unpacking
    reg [2:0]  pf_row;          // Current row being filled
    reg [3:0]  pf_col;          // Column offset (advances by 4 per word)

    // =========================================================================
    // BLOCK A — COMBINATIONAL OUTPUTS
    // =========================================================================
    
    assign active_buffer = {1'b0, active_buf};
    assign fill_level_a  = words_in_a;
    assign fill_level_b  = words_in_b;

    // Buffer status: Ready if the active side has valid data
    assign buffer_ready = (active_buf == 1'b0) ? buffer_a_valid : buffer_b_valid;
    assign buffer_empty = !buffer_ready;

    // Prefetch status
    assign prefetch_done = (pf_state == PF_DONE);
    assign prefetch_busy = (pf_state != PF_IDLE) && (pf_state != PF_DONE);
    assign mem_busy      = prefetch_busy;

    // Output Data Multiplexing
    // We drive N wires to the systolic array simultaneously.
    genvar i;
    generate
        for (i = 0; i < N; i = i + 1) begin : weight_output
            assign weight_out[i] = (active_buf == 1'b0) ? buffer_a[drain_row][i] : buffer_b[drain_row][i];
        end
    endgenerate

    // Row Selection (One-Hot)
    // Tells the array: "Load these weights into Row K".
    // Example: if drain_row=2, select is 8'b0000_0100.
    assign weight_row_select = (drain_enable && buffer_ready) ? (8'b1 << drain_row) : 8'b0;
    assign weight_valid      = drain_enable && buffer_ready;


    // =========================================================================
    // BLOCK C — DRAIN & SWAP LOGIC
    // =========================================================================
    // This process advances the row pointer and manages the ping-pong swap.
    
    always @(posedge clk) begin
        if (reset) begin
            drain_row      <= 3'b0;
            active_buf     <= 1'b0;
            buffer_a_valid <= 1'b0;
            buffer_b_valid <= 1'b0;
            words_in_a     <= 6'd0;
            words_in_b     <= 6'd0;
        end else begin
            if (drain_enable && buffer_ready) begin
                if (drain_row_done) begin
                    // If we just finished the last row (N-1), it's time to swap
                    if (drain_row >= N - 1) begin
                        drain_row <= 3'b0;
                        
                        // Invalidate the buffer we just finished
                        if (active_buf == 1'b0) begin
                            buffer_a_valid <= 1'b0;
                            words_in_a     <= 6'd0;
                        end else begin
                            buffer_b_valid <= 1'b0;
                            words_in_b     <= 6'd0;
                        end

                        // Swap to the other side ONLY if it has been prefetched
                        if ((active_buf == 1'b0 && buffer_b_valid) || (active_buf == 1'b1 && buffer_a_valid)) begin
                            active_buf <= ~active_buf;
                        end
                    end else begin
                        drain_row <= drain_row + 1'b1;
                    end
                end
            end
        end
    end


    // =========================================================================
    // BLOCK B — PREFETCH FSM (DMA ENGINE)
    // =========================================================================
    // This process runs in the background to fill the non-active buffer.

    always @(posedge clk) begin
        if (reset) begin
           pf_state         <= PF_IDLE;
           pf_addr          <= 14'd0;
           pf_word_count    <= 6'd0;
           pf_total_words   <= 6'd0;
           pf_target_buffer <= 1'b0;
           pf_row           <= 3'b0;
           pf_col           <= 4'd0;
           mem_req          <= 1'b0;
           mem_addr         <= 14'd0;
        end else begin
            case (pf_state)
                
                PF_IDLE: begin
                   mem_req <= 1'b0;
                   if (prefetch_start) begin
                        pf_addr        <= prefetch_base_addr;
                        // Each 32-bit word fills 4 columns. Total words = Rows * (N/4)
                        pf_total_words <= prefetch_rows * (N/4);
                        pf_word_count  <= 6'd0;
                        pf_row         <= 3'b0;
                        pf_col         <= 4'd0;
                        // Targeted prefetch: always fill the buffer that the array ISN'T using.
                        pf_target_buffer <= ~active_buf;
                        pf_state       <= PF_REQUEST;
                   end
                end

                PF_REQUEST: begin
                    mem_req  <= 1'b1;
                    mem_addr <= pf_addr;
                    pf_state <= PF_WAIT;
                end

                PF_WAIT: begin
                    mem_req <= 1'b0;
                    if (mem_valid) begin
                        pf_state <= PF_STORE;
                    end
                end

                PF_STORE: begin
                    // Unpack 4 weights from one 32-bit word
                    if (pf_target_buffer == 1'b0) begin
                        buffer_a[pf_row][pf_col]   <= mem_rdata[7:0];
                        buffer_a[pf_row][pf_col+1] <= mem_rdata[15:8];
                        buffer_a[pf_row][pf_col+2] <= mem_rdata[23:16];
                        buffer_a[pf_row][pf_col+3] <= mem_rdata[31:24];
                    end else begin
                        buffer_b[pf_row][pf_col]   <= mem_rdata[7:0];
                        buffer_b[pf_row][pf_col+1] <= mem_rdata[15:8];
                        buffer_b[pf_row][pf_col+2] <= mem_rdata[23:16];
                        buffer_b[pf_row][pf_col+3] <= mem_rdata[31:24];
                    end

                    pf_addr       <= pf_addr + 1'b1;
                    pf_word_count <= pf_word_count + 1'b1;
                    
                    // Update column/row position
                    if (pf_col + 4 >= N) begin
                        pf_col <= 4'd0;
                        pf_row <= pf_row + 1'b1;
                    end else begin
                        pf_col <= pf_col + 4;
                    end

                    // Check if done
                    if (pf_word_count + 1 >= pf_total_words) begin
                        pf_state <= PF_DONE;
                    end else begin
                        pf_state <= PF_REQUEST;
                    end
                end

                PF_DONE: begin
                    if (pf_target_buffer == 1'b0) begin
                        buffer_a_valid <= 1'b1;
                        words_in_a     <= pf_total_words;
                    end else begin
                        buffer_b_valid <= 1'b1;
                        words_in_b     <= pf_total_words;
                    end
                    pf_state <= PF_IDLE;
                end

                default: pf_state <= PF_IDLE;

            endcase            
        end
    end

    // =========================================================================
    // BLOCK D — SIMULATION INIT
    // =========================================================================
    integer r,c;
    initial begin
        for (r = 0; r < N; r = r + 1) begin
            for (c = 0; c < N; c = c + 1) begin
                buffer_a[r][c] = 8'sd0;
                buffer_b[r][c] = 8'sd0;
            end
        end
    end

endmodule